...
// Delaunay Triangulation by Arne Maus 2014 - 2015
...
class DelaunayAlg1 {
	...
	void PerformAlg(String [] args){
		...
		cTime = (double) (System.nanoTime()-t)/1000000.0;
		println("\nTid: Cohull fant:"+theCoHull.numElem+" punkter p� cohull. Tid:" + Format.align(cTime,10,3)+" msek, dvs. "
		...
		println("\nDT-kanter funnet:"+ (numNNeigh/2)+ " unike med " +NUM_PER_BOX+" gj.snitt punkter per box p� "  +
		...
		if (n< DebugLimit)   dumpPoints("Etter n�rmeste nabo");
	...
	double[] kotetrekking(int punkt1, int punkt2, int kote){
		...
		if(zMax < kote || zMin > kote) return new double[1]; //kan ikke trekke kote når den ikke finnes


		double[] koordinater = new double[2];
		double a = beregnDistanseMellomToPunkt(punkt1, punkt2); //
	...
	**********************************************************************/
	...
	/******************************************************************
	*	Alg 1: Finn f�rst n�rmeste nabo b for punkt a, s�
	*           finn neste nabo C til A, dvs. trtekanten ABC ved:
	*		1. F�rst f�rst den C med minst (lavest, gjerne  neg verdi) p�
	*		   skj�ring mellom midtnormalene AB og AC blant alle punkter
	*			inne i de bokser som dekker A og B.
	*		2. Test s� om funnet C er inne i sirkel som er inkludert i
	*		   de boksene det er s�kt i  og 'over' AB.
	*		   a) Ok , C er funnet
	*		   b) NEI, utvid s�kebokser til de dekker Sirkel ABC
	* 				og s�k i resten av dem (etter evt bedre C)
	********************************************************************/
	...
	// Find and add all  neighbours to ''a'from start DK ab.
	// Return when next neighbor is stop
	// When calling this method, only b has been added as
	// neighbour to point a.
	...
	// **************************************************************
	// find next Delaunay neighbor c from line a-b
	...
	int findNextNeighbour(int a, int b) {
		...
		do {
			...
			// l�p gjennom om�det og finn den beste c som
			for (int xx = lowx; xx <= highx; xx++) {
				...
				for (int p = box[xx][lowy]; p < box[xx][highy+1];  p++) {
					if (p != a && p!=b) {
						...
						if ( radp2 <= sr2 &&  cAbovepB(p, lineAB)) {
							...
							//println(" ------ C1, Fant brukbar p:"+p+", n�v�rende c:"+c);
